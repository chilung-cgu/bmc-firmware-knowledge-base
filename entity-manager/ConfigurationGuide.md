...
| Type | 說明 | D-Bus 路徑前綴 |
|------|-----|---------------|
| `Board` | 電路板（主機板、子卡） | `.../board/{Name}` |
| `Chassis` | 機箱 | `.../chassis/{Name}` |
| `PowerSupply` | 電源供應器 | `.../powersupply/{Name}` |
| `CPU` | 處理器 | `.../cpu/{Name}` |
| `Fan` | 風扇 | `.../fan/{Name}` |
...
| `$address` | FruDevice.ADDRESS | I2C 裝置位址 |
| `$index` | 匹配順序 | 多重匹配時的索引（0, 1, 2...） |
| `${PROPERTY}` | 任意 D-Bus 屬性 | 從匹配物件取得的屬性值 |
