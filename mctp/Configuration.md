...
> ❓ **尚未實作**：當前 upstream `mctpd.c` 中僅有 TODO 註解（line ~4985），此配置選項不會被解析。

橋接器下游端點的輪詢間隔。設計上啟用後，mctpd 會定期對橋接器 EID 池中的每個 EID 發送 Get Endpoint ID 命令。

| 項目             | 值                        |
| ---------------- | ------------------------- |
| **型別**         | integer（毫秒）           |
| **預設值**       | 0（禁用）                 |
| **規劃有效範圍** | 2500-10000（設為 0 禁用） |
...
> [!NOTE]
> 此功能在當前 upstream 中尚未實作（`mctpd.c` line ~4985 僅有 `// TODO: Polling logic for downstream EID`）。以下說明為規劃設計。
