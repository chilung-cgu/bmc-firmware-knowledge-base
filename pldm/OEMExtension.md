...
| Meta     | `oem-meta`   | `oem/meta/`   | Meta OEM 支援 (特定分支/下游擴充)   |
| NVIDIA   | `oem-nvidia` | `oem/nvidia/` | NVIDIA OEM 支援 (特定分支/下游擴充) |
...
```mermaid
graph TB
    subgraph Interfaces["OEM 抽象介面<br/>(libpldmresponder/oem_handler.hpp)"]
    ...
    subgraph Implementations["各廠商實作"]
        IBM["OemIBM"]
        Ampere["OemAMPERE"]
        Meta["OemMETA<br/>(特定分支)"]
        NVIDIA["OemNVIDIA<br/>(特定分支)"]
    end
